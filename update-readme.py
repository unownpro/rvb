...
for key, info in build_data.items():
    ...
    match = re.search(details_pattern, readme, re.IGNORECASE | re.DOTALL)
    if not match:
        continue
    ...
    for variant in variants:
        ...
        if variant.endswith(".apk"):
            new_file = f"{name}-v{link_version}-{variant}"
        else:
            new_file = f"{name}-module-v{link_version}-{variant}"

        link_pattern = (
            rf'((?:\.\./)*releases/download/)[^\s)"\'<>]*-{re.escape(variant)}'
        )
